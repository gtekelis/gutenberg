...
On each Gutenberg page load, we register an action that collects the meta box data to determine if an
...
retrieve the metaboxes HTML from the prefetch location and watches input and changes.
...
When the post is updated, only meta boxes areas that are active and dirty, will be
...
If the metabox area is saving, we display an updating overlay, to prevent users from
changing the form values while the meta box is submitting.
...
`gutenberg_meta_box_save()` is used to save the meta boxes changes. A `meta_box`
request parameter should be present and should match one of `'advanced'`, `'normal'`,
or `'side'`. This value will determine which meta box area is served.
...
Thus page page mimics the `post.php` post form, so when it is submitted it will
...
any existing code. On successful submission, React will signal a `handleMetaBoxReload`
